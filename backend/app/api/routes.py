...
class TicketCreateSchema(BaseModel):
    ...
    def strip_and_reject_control_chars(cls, value: str) -> str:
        if "\x00" in value:
            raise ValueError(
                "Station name must not contain null characters (0x00). "
                "This usually indicates a corrupted request payload."
            )
        cleaned = value.strip()
        if not cleaned:
...
def purchase_ticket(ticket_data: TicketCreateSchema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=400, detail=f"Invalid ticket data: {str(e.orig) if e.orig else str(e)}")
    except Exception as e:
        db.rollback()
